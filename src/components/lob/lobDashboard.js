...
export default function LobDashboard(props) {
        ...
        await apis.bank
            ...
            .then((res) => {
                ...
                if (res.status === 201) {
                    setaccessStatus({
                        message:
                            "Request The User To Give Access To The Documents",
                        disabled: false,
                    ...
                    setaccessStatus({
                        message:
                            "Requested Customer for Access, Yet to be Accepted",
                        disabled: true,
                    ...
                    setaccessStatus({
                        message: "Customer has rejected the access request",
                        disabled: false,
                    ...
                    setaccessStatus({
                        message: "Customer has accepted access request",
                        disabled: true,
    ...
    const viewKYCDoc = async (event, email, type) => {
        ...
        if (flag === 1) {
            try {
                ...
                //     let progress =
                //         Math.round(
                //             (Math.atan(currentProgress) / (Math.PI / 2)) *
                //             100 *
                //             1000
                //         ) / 1000;
    ...
    const requestPermission = async (event, email) => {
        ...
        if (response.status === 200) {
            setaccessStatus({
                message: "Requested Customer for Access, Yet to be Accepted",
                disabled: true,
                buttonMessage: "Pending",
                type: "warning",
                access: false,
